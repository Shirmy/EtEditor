...
class FetchCatalogRuleUtilsTest {
    ...
    @Test
    fun parseReadsCategoryActionEnabledWithDefaults() {
        val json = """
            [
              {"name":"去括号","search":"【.*?】","replacement":"","regex":true,"category":"净化"},
              {"name":"滤单章","search":"单章","action":"drop","category":"章节","enabled":false}
            ]
        """.trimIndent()
        ...
        assertEquals(FETCH_CATALOG_RULE_ACTION_REPLACE, items[0].action)
        assertTrue(items[0].regex)
        assertTrue(items[0].enabled)
        assertEquals(FETCH_CATALOG_RULE_CATEGORY_CHAPTER, items[1].category)
        assertEquals(FETCH_CATALOG_RULE_ACTION_DROP, items[1].action)
    ...
    fun serializeRoundTripsThroughParse() {
        val items = listOf(
            FetchCatalogRuleItem(0, FETCH_CATALOG_RULE_CATEGORY_CHAPTER, "去序号", "^\\d+\\s+", "", true, FETCH_CATALOG_RULE_ACTION_REPLACE, true),
            FetchCatalogRuleItem(1, FETCH_CATALOG_RULE_CATEGORY_PURIFY, "滤公告", "公告", "", false, FETCH_CATALOG_RULE_ACTION_DROP, false)
        )
    ...
    fun addAppendsRuleWithGivenCategory() {
        val text = addFetchCatalogRule("", FETCH_CATALOG_RULE_CATEGORY_PURIFY, "去括号", "【.*?】", "", true, FETCH_CATALOG_RULE_ACTION_REPLACE)
    ...
    fun updateReplacesFieldsAtIndex() {
        val text = addFetchCatalogRule("", FETCH_CATALOG_RULE_CATEGORY_PURIFY, "a", "x", "", false, FETCH_CATALOG_RULE_ACTION_REPLACE)
        val updated = updateFetchCatalogRule(text, 0, FETCH_CATALOG_RULE_CATEGORY_CHAPTER, "b", "y", "z", true, FETCH_CATALOG_RULE_ACTION_DROP)
        ...
        assertEquals(FETCH_CATALOG_RULE_ACTION_DROP, item.action)
    }
    ...
    fun deleteRemovesRuleAtIndex() {
        var text = addFetchCatalogRule("", FETCH_CATALOG_RULE_CATEGORY_PURIFY, "a", "x", "", false, FETCH_CATALOG_RULE_ACTION_REPLACE)
        text = addFetchCatalogRule(text, FETCH_CATALOG_RULE_CATEGORY_PURIFY, "b", "y", "", false, FETCH_CATALOG_RULE_ACTION_REPLACE)
